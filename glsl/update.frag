...
vec2 updatePosition(inout vec2 p, inout vec2 v, vec2 obstacle) {
    p += v;
...
vec2 updateVelocity(inout vec2 p, inout vec2 v, vec2 obstacle) {
    v.y -= gravity;
    if (p.y + v.y < -1.0) {
        /* Left the world, reset particle. */
...
void main() {
    ...
    float s;
    if (derivative == 0) {
        result = updatePosition(p, v, obstacle);
        s = scale.x;
    } else {
        result = updateVelocity(p, v, obstacle);
        s = scale.y;
    }
